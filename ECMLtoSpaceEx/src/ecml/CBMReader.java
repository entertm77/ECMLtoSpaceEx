...
public class CBMReader {
	...
	private void setConnection(Node varnode){
		...
		setBasicAttr(connection, varnode);
		
		connection.setIs_middle_label(getAttrValue(varnode, "is_middle_label"));
		connection.setMiddle_content(getAttrValue(varnode,"middle_content"));
		...
		connection.setSource_id(Long.parseLong(getAttrValue(varnode, "source_id")));
		connection.setTarget_id(Long.parseLong(getAttrValue(varnode, "target_id")));
		cbm.addConnection(connection);
	}
